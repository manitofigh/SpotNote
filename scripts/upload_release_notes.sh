#!/usr/bin/env bash
# Upload generated Sparkle release notes HTML to R2.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT_DIR"

if [[ -f "$ROOT_DIR/.env" ]]; then
  set -a
  # shellcheck disable=SC1091
  source "$ROOT_DIR/.env"
  set +a
fi

require_cmd() { command -v "$1" >/dev/null 2>&1 || { echo "ERROR: missing command: $1" >&2; exit 1; }; }
require_env() { [[ -n "${!1:-}" ]] || { echo "ERROR: missing env var: $1" >&2; exit 1; }; }

require_cmd aws
require_env AWS_ACCESS_KEY_ID
require_env AWS_SECRET_ACCESS_KEY
require_env AWS_ENDPOINT_URL
require_env AWS_DEFAULT_REGION

BUCKET="${SPOTNOTE_R2_BUCKET:-spotnote}"
UPDATES_BASE_URL="${SPOTNOTE_UPDATES_BASE_URL:-https://updates.spotnote.org}"

NOTES_PATH="${1:-$(ls -t "$ROOT_DIR"/dist/updates/release-notes-*.html 2>/dev/null | head -n 1 || true)}"
if [[ -z "$NOTES_PATH" || ! -f "$NOTES_PATH" ]]; then
  echo "ERROR: release notes not found. Run ./scripts/release.sh first." >&2
  exit 1
fi

NOTES_FILENAME="$(basename "$NOTES_PATH")"
DESTINATION="s3://$BUCKET/$NOTES_FILENAME"
echo "Uploading release notes:"
echo "  source: $NOTES_PATH"
echo "  target: $DESTINATION"

aws s3 cp \
  "$NOTES_PATH" \
  "$DESTINATION" \
  --endpoint-url "$AWS_ENDPOINT_URL" \
  --content-type "text/html; charset=utf-8" \
  --cache-control "no-cache, no-store, must-revalidate"

echo "Done. Public URL: $UPDATES_BASE_URL/$NOTES_FILENAME"
